...
@section('content')
    ...
    @php
    $heads = [['label' => 'ID', 'width' => 5], 'Título', 'Valor', 'Status', 'Visualizações', ['label' => 'Ações', 'no-export' => true, 'width' => 10]];

    $list = [];

    foreach ($products as $product) {
        $list[] = [$product->id, $product->title, $product->sale_price, $product->status, $product->views, '<nobr>' . '<a class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar" href="products/' . $product->id . '/edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>' . '<a class="btn btn-xs btn-default text-danger mx-1 shadow" title="Excluir" href="products/destroy/' . $product->id . '" onclick="return confirm(\'Confirma a exclusão deste produto?\')"><i class="fa fa-lg fa-fw fa-trash"></i></a>'];
    }
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><i class="fas fa-fw fa-cart-plus"></i> Produtos</h1>
                </div>
                <div class="col-sm-6">
