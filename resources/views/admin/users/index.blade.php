...
@section('content')
    ...
    @php
    $heads = [['label' => 'ID', 'width' => 5], 'Nome', 'E-mail', ['label' => 'Ações', 'no-export' => true, 'width' => 10]];

    $list = [];

    foreach ($users as $user) {
        $list[] = [$user->id, $user->name, $user->email, '<nobr>' . '<a class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar" href="users/' . $user->id . '/edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>' . '<a class="btn btn-xs btn-default text-danger mx-1 shadow" title="Excluir" href="users/destroy/' . $user->id . '" onclick="return confirm(\'Confirma a exclusão desta usuário?\')"><i class="fa fa-lg fa-fw fa-trash"></i></a>'];
    }
